...
public class PrepPhrase {
	...
	public void setPlace(NounPhrase a) {
		place = a;
	}

	public void setPrepPhrase(PPPhraseSpec a) {
		prepPhrase = a;
	}

	public void setClassPreposition(String a) {
		classPreposition = a;
	}
	...
	public PrepPhrase(VerbPhrase a) {
		try {
			int prepI = (int) (Math.random() * posPreps(a.getClassVerb()).split("--").length);// prepI = instance of
																								// prep in verb Def
			classPreposition = ranPrep(posPreps(a.getClassVerb()).split("--")[prepI].split("-")[0].split(","), true); // prepS
																														// =
																														// preposition
																														// String
			place = new NounPhrase(
					Noun.ranNoun(posPreps(a.getClassVerb()).split("--")[prepI].split("-")[1].split(","), true));// prepositional
																												// phrase
			prepPhrase = nlgFactory.createPrepositionPhrase();
			prepPhrase.setDeterminer(Noun.ranDet());
			prepPhrase.addComplement(place.toNPPhraseSpec());
			prepPhrase.setPreposition(classPreposition.split(" ")[1]);
		} catch (IOException ex) {
		}
	...
	public PrepPhrase(VerbPhrase a, NounPhrase b) {
		try {
			...
			if ((a.getClassVerb()).contains("/"))
				place = new NounPhrase(
						Noun.ranNoun(posPreps(a.getClassVerb()).split("--")[prepI].split("-")[1].split(","),
								Noun.upGroups(b.getClassNoun()), true));// prepositional phrase//WORK
			if (place == null) {
	...
	// Pre: a = array of posible indexes of prepositions; Whole determines whether
	// it is full identification(t = full)
